...
import DataConsensusTable from './DataConsensusTable';

/**
 * Page component for displaying trader epoch data consensus details
 */
export default function TraderEpochDataConsensusPage() {
  ...
  return (
    <Stack direction='column' spacing={0} sx={{ width: '80%', margin: '0 auto' }}>
      <Stack direction='column' spacing={2}>
        <Box sx={{ py: 4 }}>
          <Breadcrumbs aria-label='breadcrumb'>
            ...
            <Typography color='text.primary'>Proof of Trading Consensus</Typography>
          </Breadcrumbs>
        </Box>
        <Box sx={{ pb: 4 }}>
          <Typography variant='h2'>Proof of Trading Consensus</Typography>
          ...
          <Stack direction='row' spacing={4} sx={{ mt: 1 }}>
            <Typography color='text.secondary' variant='body1'>
              Block started {formatEpochTime(epochStart)} ({moment.unix(epochStart).fromNow()})
            </Typography>
          </Stack>
        </Box>
      </Stack>
      <AccountsProvider>
        ...
        >
          <DataConsensusTable config={config} epoch={epoch} traderId={traderId} />
        </Box>
      </AccountsProvider>
    </Stack>
  );
